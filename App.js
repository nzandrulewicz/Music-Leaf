...
function App() {
  ...
  const renderRecommendedSongs = () => {
    return recommendedSongs.map(recommendedSong => (
      <div key={recommendedSong.id} className='p-1 add-space shadow p-3 mb-5 bg-white rounded' id='img__container'>
        <br></br>
          ...
          <a href={`${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=${scopes.join("%20")}&response_type=${RESPONSE_TYPE}`}>Login to Spotify</a>
            ...
            <Container id='main__container'>
              <div onLoad={getRecentlyPlayedSong}>
                {currentUsersProfile &&
                  <div>
                    <p id='welcome__saying'>Welcome, {currentUsersProfile.display_name}</p>
                    {currentUsersProfile.images.length && currentUsersProfile.images[0].url && (
                      <img className='p-1 add-space shadow p-3 mb-5 bg-white rounded' src={currentUsersProfile.images[0].url} alt="Avatar"/>
                    )}
                  </div>
                }
              </div>
                ...
                <Button variant="success" size="lg" className="shadow p-2 mb-5" onClick={getRecommendedSongs}>Get Recommended Songs</Button>

                <br></br>

                <Button variant='danger' className='shadow p-2 mb-5' onClick={logout}>Logout</Button>
